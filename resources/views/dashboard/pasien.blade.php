        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-2 space-y-6">
                        ...
                        @if ($latestAppointment)
                            @php
                                $statusClass = [
                                    ...
                                    'Rejected' => 'bg-red-500',
                                    'Selesai' => 'bg-blue-500',
                                ][$latestAppointment->status] ?? 'bg-gray-500';
                            @endphp
                            
                            <div class="p-4 border border-gray-700 rounded-lg">
                                <span class="{{ $statusClass }} text-white text-xs font-semibold px-3 py-1 rounded-full mb-3 inline-block">{{ $latestAppointment->status }}</span>
                    ...
                    <div class="bg-gray-800 card-glass p-6 overflow-hidden shadow-xl rounded-lg">
                        <h3 class="text-lg font-bold text-yellow-400 mb-4">Notifikasi Resep</h3>
                        @if ($hasReadyPrescription)
                            <div class="p-3 bg-yellow-900/50 border border-yellow-500 rounded-lg">
                                <p class="text-white font-semibold">Resep baru siap diambil!</p>
                                <p class="text-xs text-yellow-300 mt-1">Lihat detail di riwayat perawatan Anda.</p>
                            </div>
                        @else
                            <p class="text-gray-400 text-sm">Tidak ada resep yang menunggu pengambilan.</p>
                        @endif
                    </div>

                    <div class="bg-gray-800 card-glass p-6 overflow-hidden shadow-xl rounded-lg">
                        <h3 class="text-lg font-bold text-purple-400 mb-4">Aksi Cepat</h3>
                        <a href="{{ route('pasien.appointments.create') }}" class="btn-primary-custom w-full block text-center mb-3">
